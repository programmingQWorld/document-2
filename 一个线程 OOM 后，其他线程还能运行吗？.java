...
1 'Java堆溢出'
    ...
    案例1
        ...
        那既然发生了，'如何解决这个内存区域的异常呢'？ 
            ...
            (lincq: 原则上，查看实例对象是否需要被回收，本应该被回收而实际没有回收属于内存泄漏)
...
3 '方法区和运行时常量池溢出'
    ...
    */
    ...
    '本机直接内存溢出'
        直接内存（Direct Memory）的'容量大小'可通过-XX:MaxDirectMemorySize指定，'若不指定'，则'默认与Java堆最大值'（-Xmx）一致。

        这里越过DirectByteBuffer类，直接'通过反射获取Unsafe实例'进行'内存分配'。 
        Unsafe类的getUnsafe()指定'只有引导类加载器'才会'返回实例'，体现了'设计者希望只有虚拟机标准类库里面的类才能使用Unsafe'，
        'JDK10时' '才将Unsafe的部分功能' 通过VarHandle开放给外部。 
        
        虽然使用DirectByteBuffer分配内存'也会抛OOM'，但'它抛异常时' '并未真正向os申请分配内存'，
