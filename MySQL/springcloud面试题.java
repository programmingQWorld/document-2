...
SpringBoot的'优点'：
    ...
    4、提供意见发展方法
    5、支持应用'默认配置'，'快速开始开发'
    6、内嵌'web服务器' tomcat/jetty/netty/undertow(不支持JSP，并发性能较高)，'spring-boot-starter-web默认使用的是tomcat服务器'。
    7、'更少的配置'，通过properties,yaml,JavaConfig等来配置应用中需要的信息。
使用经验：
    1.配置文件不打到fat-jar中，在需要修改配置时候，就不需要重新编译打包，而是修改外部配置文件，重新启动机器即可。

'SpringBoot常用的starter'：
    1、spring-boot-starter-web(默认嵌入Tomcat和web开发需要的servlet和jsp支持)
...
Springcloud解决那些问题：
'配置管理'、'服务发现、服务注册'、断路器、路由策略、全局锁、分布式会话、客户端调用、接口网关（zuul）、服务管理系统
SpringBoot与Springcloud：
    SpringBoot'简化了xml配置，快速整合框架'
    Springcloud是一套'微服务解决方案'—'RPC远程调用'
    关系Springcloud依赖与SpringBoot（web组件用的SpringMVC），为什么Springcloud会依赖与SpringBoot？因为Springcloud'写接口就是SpringMVC接口'
    SpringBootproperties和yml中可以使用${random}设置一些随机值
...
微服务就是'将工程根据不同的业务规则拆分成微服务'，部署'在不同的服务器上'，'服务之间相互调用'，
java中有的'微服务有dubbo'(只能用来做微服务)、'springcloud'( 提供了服务的发现、断路器等)。

微服务的特点：
    '按业务划分为一个个独立运行的程序'，即服务单元
    服务之间'通过HTTP协议相互通信',可以用不同的编程语言,可以用不同的存储技术
    '自动化部署'

微服务的优势：
    1、将一个复杂的业务拆分'为若干小的业务'。因为'代码体积变小'，'逻辑更加专一'，'接手成本较低'。
    2、微服务系统的'微服务单元具有很强的横向拓展能力'。
...
雪崩效应：
    ...
    如果'一个服务之间出现了故障或者网络延迟'，会导致'调用方线程阻塞直到接口返回'，在高并发的情况下，该'服务消费方的线程资源会消耗殆尽'，最终'使得该服务不可用'。
...
'Eureka和zookeeper'都可以提供'服务注册与发现'的功能，'两者的区别'：'Zookeeper保证了CP'(C：'一致性'，P：'分区容错性')，'Eureka保证了AP'(A：高可用，P：分区容错)
    ...
    1、Zookeeper-----当'向注册中心查询服务列表'时，我们可以'容忍注册中心返回的是几分钟以前的信息'，但'不能容忍直接down掉不可用的'。（启用熔断器不是就可以吗？）
...
服务注册与发现：
    服务注册就是'向服务注册中心注册一个服务实例'，'服务提供者'将自己的'服务信息'（服务名、IP地址等）'告知注册中心'。
    服务发现是'服务消费另一个服务'时，注册中心将服务的实例'返回给服务消费者'，一个服务既是服务提供者又是服务消费者。
